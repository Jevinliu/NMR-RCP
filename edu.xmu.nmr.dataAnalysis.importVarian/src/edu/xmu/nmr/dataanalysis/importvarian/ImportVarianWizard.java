	...
	protected void findKeySet(String directoryPath,
		...
		if (leftToRight.containsKey(directoryPath)) {
			...
			for (TreeObject rightObj : rightRoot.getChildren()) {
				if (rightObj.isChecked() && judgeFileIsNeed(rightObj.getName())) {
					if (rightObj.getName().equalsIgnoreCase("procpar")) {
						...
						VersionNode etp = new VersionNode(parent.getNodePath()
								+ File.separator + version, FileType.VARIAN,
								version);
						parent.addChild(etp);
						FidNode fid = new FidNode(etp.getNodePath()
								+ File.separator + "fid", FileType.BRUKER,
								"fid");
						ParamtersNode params = new ParamtersNode(
								etp.getNodePath() + File.separator
										+ "paramters", FileType.BRUKER);
						etp.addChild(fid);
						etp.addChild(params);
